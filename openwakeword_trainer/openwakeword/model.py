...
class Model():
    ...
    def __init__ (
        ...
        Parameters:
            wakeword_models (`list[str]`):
                A list of paths of ONNX/tflite models to load into the
                openWakeWord model object. If not provided, will load all of
                the pre-trained models. Alternatively, just the names of
                pre-trained models can be provided to select a subset of
                models.
            vad_threshold (`float`):
                ...
                voice activity detection entirely.
            custom_verifier_models (`dict[str, str]`):
                A dictionary of paths to custom verifier models, where the keys
                are the model names (corresponding to the openwakeword.MODELS
                attribute) and the values are the filepaths of the custom
                verifier models.
        ...
        self.prediction_buffer: DefaultDict[str, deque] = defaultdict(ft.partial(deque, maxlen=30))
    ...
    ### HELPERS ###
    def _suppress_noise_with_speex(
                self,
        ...
        Parameters:
            x (`ndarray`):
                The 16-bit, 16khz audio to process. Must always be an integer
                multiple of `frame_size`.
            frame_size (`int`):
                The frame size to use for the Speex Noise suppressor. Must
                match the frame size specified during the initialization of the
                noise suppressor.

        Returns:
            suppressed (`ndarray`):
                The input audio with noise suppression applied.
        '''
        cleaned = []
